// src/features/builder/tabs/tx-body/inputs/TransactionInputsTab.tsx
'use client';
...
export function TransactionInputsTab() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.success('Transaction input added');
      
      setTxId('');
      setIndex('');
      setErrors({});
    } catch (error) {
      toast.error(`Failed to add input: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>
          Add transaction inputs (UTXOs) to consume
        </CardDescription>
      </CardHeader>
