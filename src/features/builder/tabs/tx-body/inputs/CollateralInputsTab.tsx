// src/features/builder/tabs/tx-body/inputs/CollateralInputsTab.tsx
'use client';
...
export function CollateralInputsTab() {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.success('Collateral input added');
      
      setTxId('');
      setIndex('');
      setErrors({});
    } catch (error) {
      toast.error(`Failed to add collateral input: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Card className="h-full">
      <CardHeader>
        ...
        <CardDescription>
          Add collateral inputs for Plutus script transactions
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        ...
        <Button
          ...
          className="w-full"
        >
          {loading ? 'Adding...' : 'Add Collateral Input'}
        </Button>
      </CardContent>
    </Card>
  );
