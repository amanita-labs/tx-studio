...
/**
 * Runs `fn`, freeing every CSL wasm object passed through `track()` once it
 * returns (or throws). CSL objects are wasm-backed and must be `.free()`d
 * explicitly or they leak for the lifetime of the worker; in a long-lived
 * worker that parses many transactions this accumulates. Wrap any block that
 * creates intermediate CSL objects purely to extract a string/value.
 *
 * Each free is individually guarded: some CSL methods throw mid-call and leave
 * an active wasm-bindgen borrow, after which `.free()` throws "recursive use of
 * an object detected" — swallowing keeps that from masking the real result.
 */
function freeing<T>(fn: (track: <O>(o: O) => O) => T): T {
  const objs: Array<{ free(): void }> = [];
  const track = <O,>(o: O): O => {
    if (o && typeof (o as { free?: unknown }).free === 'function') {
      objs.push(o as unknown as { free(): void });
    }
    return o;
  };
  try {
...
/**
 * Derives the enterprise (payment-only) bech32 address for a script hash,
 * freeing the intermediate CSL objects. Throws if CSL rejects the hash; callers
 * keep their own try/catch for the warning message.
 */
function scriptHashToEnterpriseAddress(hash: string, networkId: number): string {
  return freeing((track) => {
    const scriptHash = track(CSL.ScriptHash.from_bytes(Buffer.from(hash, 'hex')));
    const credential = track(CSL.Credential.from_scripthash(scriptHash));
    const enterpriseAddr = track(CSL.EnterpriseAddress.new(networkId, credential));
    return track(enterpriseAddr.to_address()).to_bech32();
  });
}

/**
 * Flattens a CSL MultiAsset (as carried on TransactionOutput amounts) into a
 * list of { policyId, assetName (hex), quantity }. Shared by transaction-output
 * and collateral-return parsing — previously duplicated at both sites. Mint uses
 * a different CSL shape (MintsAssets, signed quantities) and is parsed separately.
 * Frees the intermediate policy/asset-name/quantity wasm objects.
 */
function parseMultiAssets(
  multiAsset: CSL.MultiAsset | undefined
): Array<{ policyId: string; assetName: string; quantity: bigint }> {
  const assets: Array<{ policyId: string; assetName: string; quantity: bigint }> = [];
  if (!multiAsset) return assets;
  return freeing((track) => {
    const keys = track(multiAsset.keys());
    for (let i = 0; i < keys.len(); i++) {
      const policyKey = track(keys.get(i));
      const policyId = policyKey.to_hex();
      const assetsMap = track(multiAsset.get(policyKey));
      if (assetsMap) {
        const assetNames = track(assetsMap.keys());
        for (let k = 0; k < assetNames.len(); k++) {
          const assetNameObj = track(assetNames.get(k));
          const assetName = Buffer.from(assetNameObj.name()).toString('hex');
          const quantity = BigInt(track(assetsMap.get(assetNameObj))?.to_str() || '0');
...
function cslCredToInfo(cred: CSL.Credential | undefined | null): AddressCredInfo | undefined {
  ...
  // Frees the KeyHash/ScriptHash objects created to read their hex. `cred`
  // itself is owned by the caller and is intentionally not tracked here.
  return freeing((track) => {
    try {
      ...
      if (k === 0) {
        const kh = track(cred.to_keyhash?.());
        if (kh) return { kind: 'key', hash: kh.to_hex() };
      } else if (k === 1) {
        const sh = track(cred.to_scripthash?.());
...
function decomposeAddress(cslAddr: CSL.Address | undefined | null): AddressCreds | undefined {
  ...
  // Frees the parsed address variants and the Credential objects read out of
  // them. cslCredToInfo only reads hex off the creds, so freeing them here is
  // safe. `cslAddr` is owned by the caller and not tracked.
  return freeing((track) => {
    try {
      const base = track(CSL.BaseAddress.from_address(cslAddr));
      if (base) {
        return {
          paymentCred: cslCredToInfo(track(base.payment_cred())),
          stakeCred: cslCredToInfo(track(base.stake_cred())),
        };
      }
    } catch {
      // fall through
    ...
    try {
      const ent = track(CSL.EnterpriseAddress.from_address(cslAddr));
      if (ent) {
        return { paymentCred: cslCredToInfo(track(ent.payment_cred())) };
      }
    } catch {
      // fall through
    }
    try {
      const reward = track(CSL.RewardAddress.from_address(cslAddr));
      if (reward) {
        return { stakeCred: cslCredToInfo(track(reward.payment_cred())) };
      }
    } catch {
      // fall through
    }
  ...
  try {
    ...
    try {
      ...
      if (context === 'stake') {
        ...
        return freeing((track) => {
          if (isKeyType) {
            const keyHash = track(CSL.Ed25519KeyHash.from_bytes(hexBytes));
            const stakeCredential = track(CSL.Credential.from_keyhash(keyHash));
            const rewardAddress = track(CSL.RewardAddress.new(effectiveNetworkId, stakeCredential));
            return track(rewardAddress.to_address()).to_bech32();
          } else {
            const scriptHash = track(CSL.ScriptHash.from_bytes(hexBytes));
            const stakeCredential = track(CSL.Credential.from_scripthash(scriptHash));
            const rewardAddress = track(CSL.RewardAddress.new(effectiveNetworkId, stakeCredential));
            return track(rewardAddress.to_address()).to_bech32();
          }
        });
...
function extractRewardAccountRef(address: any): RewardAccountRef | null {
  ...
  try {
    // Frees the RewardAddress/Credential/hash objects created here. cslAddress
    // may be caller-owned (when an Address object was passed in), so it is not
    // tracked — only the objects this block creates.
    return freeing((track) => {
      const rewardAddress = track(CSL.RewardAddress.from_address(cslAddress));
      if (!rewardAddress) return null;

      const intrinsicNetworkId = cslAddress.network_id();
      const cslCred = track(rewardAddress.payment_cred());
      const keyHash = track(cslCred.to_keyhash());
      const scriptHash = track(cslCred.to_scripthash());
...
/**
 * Parses a single CSL Certificate into a { type, details } view-model. Extracted
 * verbatim from the certificate loop in parseTransaction — a 10+ branch dispatch
 * over the Shelley→Conway certificate kinds. `networkId` drives bech32 credential
 * encoding (CIP-0129 HRPs) inside normalizeCredential.
 */
function parseCertificate(
  ...
  try {
    ...
    // CSL types this as a strict CertificateJSON union; the original inline code
    // accessed it loosely (via `any`), so we keep that behavior with a loose cast.
    const certJson = cert.to_js_value() as unknown as Record<string, Record<string, unknown>>;
...
/**
 * Resolves a governance action's parent (previous) action reference into a
 * CIP-0129 `gov_action1...` string. Accepts a string id or a
 * { transaction_id, index } object. Hoisted out of parseGovernanceAction.
 */
function extractParentActionId(parentActionId: any): string | null {
...
/**
 * Maps a Conway governance_action JSON object to a { type, details } view-model.
 * Extracted verbatim from the voting-proposals loop in parseTransaction — a 7-branch
 * dispatch over the action kinds. Each branch tolerates both the bare name
 * (`ParameterChange`) and the `*Action` suffix CSL sometimes emits. Returns
 * { type: 'Unknown', details: {} } when no branch matches.
 */
function parseGovernanceAction(action: Record<string, any>): { type: string; details: Record<string, unknown> } {
  ...
  return { type: proposalType, details };
}

// Real CSL-based transaction parsing
export async function parseTransaction(hex: string, network: 'mainnet' | 'preprod' | 'preview' = 'mainnet') {
  ...
  try {
    ...
    for (let i = 0; i < bodyOutputs.len(); i++) {
      ...
      // Parse assets
      const assets = parseMultiAssets(amount.multiasset());
    ...
    if (bodyCerts) {
      ...
      for (let i = 0; i < bodyCerts.len(); i++) {
        certs.push(parseCertificate(bodyCerts.get(i), networkId));
      }
    }
            ...
            if (proposalsJs.length > 0) {
              proposalsJs.forEach((proposal: any, index: number) => {
                ...
                if (proposal.governance_action) {
                  const parsedAction = parseGovernanceAction(proposal.governance_action);
                  proposalType = parsedAction.type;
    ...
    if (nativeScripts) {
      for (let i = 0; i < nativeScripts.len(); i++) {
        ...
        const hash = freeing((track) => track(script.hash()).to_hex());
        let address: string | undefined;
        let addressCreds: AddressCreds | undefined;
        try {
          address = scriptHashToEnterpriseAddress(hash, networkId);
    ...
    if (bodyWithdrawals) {
      ...
      for (const r of redeemers) {
        if (r.purpose === 'reward' && r.index < wdKeys.len()) {
          try {
            freeing((track) => {
              const rewardAddr = track(CSL.RewardAddress.from_address(track(wdKeys.get(r.index).to_address())));
              if (rewardAddr) {
                const cred = track(rewardAddr.payment_cred());
                const sh = track(cred.to_scripthash());
    ...
    if (vkeys) {
      ...
      const getHashFromVkey = (vkeyObj: any): string | null => freeing((track) => {
        try {
          ...
          if (vkeyObj) {
            ...
            if (typeof vkeyObj.hash === 'function') {
              try {
                const keyHash = track(vkeyObj.hash());
            ...
            if (typeof vkeyObj.public_key === 'function') {
              try {
                const publicKey = track(vkeyObj.public_key());
                if (publicKey && typeof publicKey.hash === 'function') {
                  const keyHash = track(publicKey.hash());
                  if (keyHash && typeof keyHash.to_hex === 'function') {
                    return keyHash.to_hex();
                  }
                }
            ...
            if (typeof vkeyObj.to_bytes === 'function') {
              try {
                ...
                if (vkeyBytes && vkeyBytes.length >= 32) {
                  const keyHash = track(CSL.Ed25519KeyHash.from_bytes(vkeyBytes.slice(0, 32)));
                  return keyHash.to_hex();
                }
              } catch (bytesError) {
                  ...
                  try {
                    freeing((track) => {
                      ...
                      if (CSL.PublicKey && typeof CSL.PublicKey.from_bech32 === 'function') {
                        const publicKey = track(CSL.PublicKey.from_bech32(vkeyJson.vkey));
                        if (publicKey && typeof publicKey.hash === 'function') {
                          const keyHash = track(publicKey.hash());
                          hash = keyHash.to_hex();
                        }
                        ...
                        if (decoded && decoded.data && decoded.data.length >= 32) {
                          ...
                          const keyHash = track(CSL.Ed25519KeyHash.from_bytes(publicKeyBytes));
                          hash = keyHash.to_hex();
                        }
                      }
                    });
...
// Structured result of a CIP-169 verification, shaped for the worker to post
// straight back to the host.
export type Cip169VerifyResult =
  | { binding: 'ok'; selectorKind: string }
  | { binding: 'mismatch'; differences: unknown }
  | { binding: 'error'; error: string; code?: string; skipped?: Array<{ kind: string; reason: string }> };

// Verify on-chain CIP-169 metadata against a transaction. Pure logic (no
// worker/postMessage coupling) so it can be unit-tested directly.
export async function verifyCip169(metadata: unknown, txCbor: string): Promise<Cip169VerifyResult> {
  try {
    ...
    if (!result.success) {
      ...
      return { binding: 'error', error: result.error.message, code: result.error.code, skipped };
    } else if (result.data.matched) {
      return { binding: 'ok', selectorKind: result.data.selectorUsed.kind };
    } else {
      return { binding: 'mismatch', differences: result.data.differences };
    }
  } catch (error) {
    return { binding: 'error', error: error instanceof Error ? error.message : 'CIP-169 verify threw' };
  }
}
